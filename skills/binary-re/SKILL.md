---
name: binary-re
description: Analyze executables, firmware and bytecode using evidence-led triage, static analysis, controlled dynamic analysis and reporting. Includes radare2/Ghidra and Python-bytecode references. Binary-parser requests for reverse engineering may use this skill.
---

# Binary Reverse Engineering


## Codex standalone packaging

Adapted from 2389-research/binary-re commit 42aee9063f3f3d52616700df3aa16df82b848604 (MIT).
This package retains the analysis guidance and reference documents. The original
Claude plugin's five sub-skills are local phase documents; read only the relevant
phase below, resolving paths against this skill directory. No plugin runtime is
required to read them. If episodic-memory tools are absent, use existing project
notes and this conversation; never invent tools or claim automatic persistence.
External analysis programs are separate dependencies. Tool setup examples require
checking the actual host; their inclusion is not a claim those programs are installed.
Use static dependency inspection for unknown binaries; do not run ldd on them.
...
|-------|---------|------------------|
| [triage](references/phases/triage.md) | Fast fingerprinting | "what is this binary", "identify", "file type" |
| [static-analysis](references/phases/static-analysis.md) | r2 + Ghidra analysis | "disassemble", "decompile", "functions" |
| [dynamic-analysis](references/phases/dynamic-analysis.md) | QEMU + GDB + Frida | "run", "execute", "debug", "trace" |
| [synthesis](references/phases/synthesis.md) | Report generation | "summarize", "report", "document findings" |
| [tool-setup](references/phases/tool-setup.md) | Install tools | "install", "setup", "tool not found" |

Read the linked phase document matching the current analysis question.
...
| macOS (any) | Docker + binfmt | See [tool-setup](references/phases/tool-setup.md) skill |
...
```
1. Triage (read references/phases/triage.md)
   └─ Architecture, ABI, dependencies, capabilities

2. Static Analysis (read references/phases/static-analysis.md)
   └─ Functions, strings, xrefs, decompilation

3. Dynamic Analysis (read references/phases/dynamic-analysis.md) - if safe
   └─ Syscalls, network, file access

4. Synthesis (read references/phases/synthesis.md)
...
| Situation | Action |
|-----------|--------|
| Tool not found | Use [tool-setup](references/phases/tool-setup.md) skill |
